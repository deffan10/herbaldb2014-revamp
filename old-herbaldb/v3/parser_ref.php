<?php session_start();
    ...
    require_once 'functions/functions.php';
		...
		for ($j = 2; $j <= $n_barisdata; $j++) {
			...
			$arr_data[ref_id]  			=   $data->sheets[0]['cells'][$j][1];
			$arr_data[ref_name]  		=   htmlspecialchars($data->sheets[0]['cells'][$j][2], ENT_QUOTES);
			$arr_data[ref_insert_by]  	=   1;
			$arr_data[ref_insert_date]	=   date("Y-m-d-h:i-a");
			$arr_data[ref_update_by]  	=   1;
			...
			*/
			...
			$sql_cek_duplicated = "select count(*) as n_all from ".$table." where ref_id='".$arr_data[ref_id]."'";
